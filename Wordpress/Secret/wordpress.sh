...
#sudo sed -i 's/localhost/password/g' /var/www/wordpress/wp-config.php
echo 'define('FS_METHOD', 'direct');' >> sudo /var/www/wordpress/wp-config.php

# Kalo pakai external mysql jangan lupa "bind-address = 0.0.0.0" di sudo nano /etc/mysql/mysql.conf.d/mysqld.cnf lalu sudo systemctl restart mysql
